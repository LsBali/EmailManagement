...
const EmployeeDashboard: React.FC = () => {
  ...
  const userMiddleName = (userDetails as any).middleName || '';
  const userLastName = (userDetails as any).lastName || '';
  const userDisplayName = userMiddleName ? `${userFirstName} ${userMiddleName} ${userLastName}` : `${userFirstName} ${userLastName}`;
  ...
  const handleSubmit = async (values: LeaveFormValues, { resetForm }: any) => {
    ...
    // Backend expects: subject, leaveReason, leaveType, startDate, endDate, attachments
    formData.append('subject', 'Leave Request Application');
    formData.append('leaveReason', values.reason);
    formData.append('leaveType', 'Sick Leave');
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-gradient-to-br from-background via-form-background to-background p-6">
          ...
          <div className="relative z-10 max-w-7xl mx-auto space-y-6">
              <div>
                <h1 className="text-3xl font-bold gradient-text">Welcome, {userDisplayName}!</h1>
